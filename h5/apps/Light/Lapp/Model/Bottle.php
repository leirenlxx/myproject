...
class Bottle extends Model
{
    ...
    public function getBottleList($appId, $page)
    {
        ...
        return $this->where($map)->alias('a')->join('LEFT JOIN jovo_bottle_user b ON a.uid = b.uid')->field('a.content,a.created,b.headimgurl,b.nickname')->order('created desc')->page($page . ',20')->select();
    }
    ...
    public function checkContent($appId, $uid, $content)
    {
        ...
        $map['content'] = $content;
        $map['status']  = 1;
        return $this->where($map)->field('uid,content,created')->find();
    }
    ...
    public function getBottleInfo($id, $uid)
    {
        ...
        $map['a.uid']    = $uid;
        $map['a.status'] = 1;
        return $this->where($map)->alias('a')->join('LEFT JOIN jovo_bottle_user b ON a.uid = b.uid')->field('a.content,a.file_path,a.created,b.nickname,b.headimgurl,a.color,a.back_img,a.choose_img,a.img,b.uid')->find();
    }
